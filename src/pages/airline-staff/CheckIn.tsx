...
export default function CheckIn() {
  ...
  const [bagDetails, setBagDetails] = useState<{ bagId: string; counter: string }[]>([]);
  const [isProcessing, setIsProcessing] = useState(false);

  // Issue modal
  const [showIssueModal, setShowIssueModal] = useState(false);
  const [issueType, setIssueType] = useState<'security_violation' | 'passenger_removal'>('security_violation');
  ...
  const handleBagCountChange = (count: string) => {
  ...
  const updateBagCounter = (index: number, counter: string) => {
